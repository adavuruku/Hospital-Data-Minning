...
$dateprint = "Printed On: ".$J.", ".$Q;	
	...
	if($_SESSION['clearance_section'] =="Library"){
		...
		$stmt_ina->execute(array($status));
		$display = "All Pending Clearance Application For Library Unit";
	}
	//store
	if($_SESSION['clearance_section'] =="Bursary"){
		...
		$stmt_ina->execute(array($status));
		$display = "All Pending Clearance Application For Bursary / Finance Department ";
	}
	//hostel
	if($_SESSION['clearance_section'] =="Hostel"){
		...
		$stmt_ina->execute(array($status));
		$display = "All Pending Clearance Application For Hostel Unit";
	}
	//athlete
	if($_SESSION['clearance_section'] =="Sports"){
		...
		$stmt_ina->execute(array($status));
		$display = "All Pending Clearance Application For Sports Unit";
	}
	//sug
	if($_SESSION['clearance_section'] =="SUG"){
		...
		$stmt_ina->execute(array($status));
		$display = "All Pending Clearance Application For Student Union Government (S U G) Unit";
	}
	//security
	if($_SESSION['clearance_section'] =="Security"){
		...
		$stmt_ina->execute(array($status));
		$display = "All Pending Clearance Application For Security Unit";
	}
	if($_SESSION['clearance_section'] =="Faculty"){
		...
		$display = "All Pending Clearance Application For Faculty of ".$_SESSION['staff_faculty'];
	}
	if($_SESSION['clearance_section'] =="Department"){
		...
		$display = "All Pending Clearance Application For Department of ".$_SESSION['staff_dept'];
	}
?>
		...
		<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12" style="text-align:centre;padding-top:10px;">
			...
			<?php
				...
				if($affected_rows_in >= 1) 
				{
					...
					echo $data.'
					<tr >
						<td colspan="12" align="center" ><h5 style="color:red">You have - '.$j.' - Pending Clearance Application </h5></td>
					</tr>
					<tr >
						...
						echo '<div class="alert alert-info alert-dismissable">
								   <button type="button" class="close" data-dismiss="alert" 
									  aria-hidden="true">
									  &times;
								   </button><h3>You dont have any Pending Clearance Application Yet..</h3></div>';
