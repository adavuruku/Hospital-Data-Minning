...
$errPL=" Edit Patient Record (Update Card Details) - All * Field are Compulsory - Make sure Every Field is Filled before Updating Record !!";
$notice_msg='<div class="alert alert-info alert-dismissable">
					   <button type="button" class="close" data-dismiss="alert" 
						  aria-hidden="true">
						  &times;
					   </button>'.$errPL.' </div>';
$txtlgov=$txtstate=$txtgender=$txtPhone=$txtoname=$txtfname=$txttitle=$txtemail=$txtstaffid=$txttempadd =$txtPermadd=$txtdob =$txtsearch=$txtout= "";
...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['search'])){
	...
	if($txtsearch != ""){
		...
		if($affected_rows_in >= 1) 
		{
			...
			$msgAll = 'Card ID No. : '.$row_two['card_id'].'</h5><br/>
						<br/><h5> Patient Name. : '.$row_two['card_name'].'</h5><br/>
			...
			$txtout='<div class="alert alert-info alert-dismissable">
					   ...
					   $txtlgov=$row_two['card_lgov'];
						$txtstate=$row_two['card_state'];$txtgender=$row_two['card_gender'];$txtPhone=$row_two['card_phone'];
						$txtfname=$row_two['card_name'];
						$txtemail=$row_two['card_email'];$txtdob=$row_two['card_dob'];
						$txtPermadd=$row_two['card_permadd'];
						$txttempadd=$row_two['card_tempadd'];
...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['submit'])){
	...
	if($txttempadd=="" || $txtPermadd=="" || $txtPhone=="" || $txtoname=="" || $txtfname=="" || $txtdob=="" || !isset($_SESSION['card_d']) ||
		$txttitle=="Select Title" || $txtgender=="Select Gender" || $txtstate=="Select State" || $txtlgov=="Select Local Government")
		{
			$err = $errPL = "Unable to Update Card Information Details.. Please Verify Your Entries to Ensure they are all Provided !!";
			$notice_msg='<div class="alert alert-danger alert-dismissable">
				...
				if($sth->execute()){
					...
					$err = $errPL = "Success: Patient Record Is Successfully Updated !!";
		...
		<div class="imageupload panel panel-primary">
			...
			<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8" style="margin-bottom:10px;margin-top:10px;text-align:centre;padding-top:10px">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 panel-primary" style="margin-bottom:5px;margin-top:5px;text-align:centre;padding-top:5px;background-color:#CCCCFF">
						...
						<h4>Enter Card ID N<u>o</u> To Search </h4>
						<hr/>
				...
				<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
					...
					<div class="col-xs-12 col-sm-12 col-md-6 col-lg-6">
						...
						<div class="form-group pull-right">
							<div class="input-group">
								<input type="submit" name="submit" style="margin-bottom:10px;padding:5px 20px 5px 20px" value="Update Card Details" class="btn btn-primary btn-md"></input>
							</div>
						</div>
					</div>
				</form>
