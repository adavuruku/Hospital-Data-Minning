...
$errPL=" Upload Diagnosis and Treatment Specification For Patient - All * Field are Compulsory !!";
$notice_msg='<div class="alert alert-info alert-dismissable">
					   <button type="button" class="close" data-dismiss="alert" 
						  aria-hidden="true">
						  &times;
					   </button>'.$errPL.' </div>';
$txtcomplain = $txtlabtest = $txtlabtestresult = $txtdiagnosis = $txtout =""; $txtsearch="";
...
if($_SERVER['REQUEST_METHOD'] == "POST" && isset($_POST['search'])){
	...
	if($txtsearch != ""){
		$txtout = "";
		$stmt_ina = $conn->prepare("SELECT * FROM treatment_history INNER JOIN patience_record ON treatment_history.card_id = patience_record.card_id where treatment_id =?  Limit 1");
		...
		if($affected_rows_in >= 1) 
		{
			...
			$dateprint_V = $J.", ".$Q;
			$dob = $J.", ".$Q;	
			$_SESSION['card_d'] = $row_two['treatment_id'];
			...
			$txtout='<div class="alert alert-info alert-dismissable">
					...
					$txtcomplain =$row_two['card_complain']; $txtlabtest =$row_two['doctor_labtest']; $txtlabtestresult =$row_two['doctor_labresult']; $txtdiagnosis = $row_two['doctor_diagnosis'];
		}else{
			if(isset($_SESSION['card_d'])){
				unset($_SESSION['card_d']);
			}
	...
	if($txtcomplain=="" || $txtdiagnosis=="" || !isset($_SESSION['card_d']))
		{
			...
			$notice_msg='<div class="alert alert-danger alert-dismissable">
				...
				$sth = $conn->prepare("UPDATE treatment_history SET staff_id=?,doctor_labresult=?,doctor_labtest=?,doctor_diagnosis=?,card_complain=?,date_record=now() where treatment_id=? Limit 1");
				if($sth->execute(array($_SESSION['staff_id'],$txtlabtestresult,$txtlabtest,$txtdiagnosis,$txtcomplain,$_SESSION['card_d']))){
					//redirect to preview page
					$_SESSION['page_authy'] = SHA1("W@YERADAVURUKUSTAS#YUR");
					$sec="Act_Me";
					//header("location: staf_account_home.php?m_=".$_SESSION['page_authy']."&l_w=".$sec);
					$err = $errPL = "Success: Treatment Record is Updated Successfully!!";
		...
		<div class="imageupload panel panel-primary">
			...
			<div class="col-xs-12 col-sm-12 col-md-8 col-lg-8" style="margin-bottom:10px;margin-top:10px;text-align:centre;padding-bottom:10px">
				...
				<div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 panel-primary" style="margin-bottom:5px;margin-top:5px;text-align:centre;padding-top:5px;background-color:#CCCCFF">
						...
						<h4>Enter Treatment ID N<u>o</u> To Search </h4>
						<hr/>
						...
						<input type="hidden" name="page_authy" value="<?php echo $_SESSION['page_authy']?>" ></input>
							<input type="hidden" name="sec" value="<?php echo $sec ?>" ></input>
							    ...
							    <div class="form-group">
									<label for="txtsearch" class="control-label col-xs-3">Enter Treatment ID N<u>o</u> : <span style="color:red">(*)</span></label>
									<div class="col-xs-6">
										<input type="text" class="form-control"  id="txtsearch" name="txtsearch" value="<?php  echo $txtsearch;?>" required="true" placeholder="Enter Treatment PIN (e.g 1234567890) " />
									</div>
									<div class="col-xs-3">
				...
				<form role="form"  name="reg_form"  id="form" class="form-vertical" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" enctype="multipart/form-data" method="POST">
					...
					<input type="hidden" name="sec" value="<?php echo $sec ?>" ></input>
						<div class="form-group">
							<label for="txtcomplain"> Patience Complain : <span style="color:red">(*)</span></label> 
							<div class="input-group">
								<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
								<textarea class="form-control"  rows="4" id="txtcomplain" name="txtcomplain" required="true" placeholder="Enter Patient Complain">
									<?php echo $txtcomplain;?>
								</textarea>
							</div>
						</div>
						
						<div class="form-group">
							<label for="txtlabtest">Laboratory Test Specification : </label>
							<div class="input-group">
								<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
								<textarea class="form-control" rows="4" id="txtlabtest" name="txtlabtest" required="true" placeholder="Enter Laboratory Test Specification">
									<?php echo $txtlabtest;?>
								</textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="txtlabtestresult">Laboratory Test Result : </label>
							<div class="input-group">
								<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
								<textarea class="form-control" rows="4" id="txtlabtestresult" name="txtlabtestresult" required="true" placeholder="Enter Residensial Address">
									<?php echo $txtlabtestresult;?>
								</textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="txtdiagnosis">Doctor Diagnosis / Treatment Specification : <span style="color:red">(*)</span></label>
							<div class="input-group">
								<span class="input-group-addon"><span class="glyphicon glyphicon-pencil"></span></span>
								<textarea class="form-control" rows="4" id="txtdiagnosis" name="txtdiagnosis" required="true" placeholder="Enter Diagnosis / Treatment Specification">
									<?php echo $txtdiagnosis;?>
								</textarea>
							</div>
						</div>
